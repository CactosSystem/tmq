...
async fn main() -> Result<()> {
    ...
    let mut publisher = xpub(&ctx).bind("tcp://127.0.0.1:5555")?;
    
    // Enable verbose mode to receive all subscription messages
    publisher.set_xpub_verbose(true)?;
    println!("Publisher bound to tcp://127.0.0.1:5555");
    ...
    let subscriber_task = tokio::spawn(async move {
        ...
        let mut subscriber = xsub(&ctx).connect("tcp://127.0.0.1:5555")?;
        println!("Subscriber connected to tcp://127.0.0.1:5555");
